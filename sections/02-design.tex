...
\begin{figure}[htbp]
  ...
  \node[block, above=1.4cm of power] (driver) {隔离驱动与保护\\UCC21540};
  \node[block, below=1.1cm of power] (sample) {采样调理};

  % Controller
  \node[bigblock, left=2.8cm of sample] (g553) {GD32G553 主控制板};

  % Battery below controller
  \node[block, below=2.5cm of sample] (bat) {16S 磷酸铁锂电池组\\BAT};

  % BMS
  \node[bigblock, below=1.95cm of g553] (bms) {GD32C113 BMS 板\\GD30BM2016 AFE};

  % WiFi and host
  \node[bigblock, right=2.5cm of sample] (wifi) {GD32VW553 无线模块};
  \node[block, below=2.5cm of wifi] (app) {上位机 / App};
  ...
  \draw[line] (g553.north west)  ++(0,0)|- node[pos=0.35, right]{PWM 驱动信号} (driver.west);
  \draw[line] (driver.south) -- node[right]{栅极驱动} (power.north);
  \draw[line] (sample.west) -- node[pos=0.5, above=0.2cm]{ADC 反馈} (g553.east);
  \draw[dline] (power.south) -- (sample.north);

  % BMS links
  \draw[line] (bat.west) -- node[below]{数据采样} (bms.east);
  \draw[line] (bms.north) -- node[pos=0.5, left=0.1cm]{CAN FD 遥测 / 保护状态} (g553.south);

  % Wireless links
  \draw[line] (g553.south) -- ++(0.2,-0.8) -| node[pos=0.4,above]{UART 数据帧} (wifi.south);
  \draw[line] (wifi.east) -- ++(0.45,0) |- node[pos=0.2,left]{WiFi} (app.east);
  \draw[dline] (app.north) -- ++(0,0) -| node[pos=0.6, left]{参数配置 / 模式命令} (wifi.south);
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{0.28\linewidth}|p{0.26\linewidth}|p{0.34\linewidth}|}
    \hline
    \rule{0pt}{1.15cm}项目 & 设计目标 & 说明 \\
    \hline
    \rule{0pt}{1.15cm}光伏输入电压 & 约 36 V 等级 & 面向低压光伏输入场景，便于与电源和光伏模拟源进行联调 \\
    \hline
    \rule{0pt}{1.15cm}储能电池电压 & 约 48 V 等级 & 单体标称电压 3.2 V，满充电压约 58.4 V，放电截止附近约 40 V \\
    \hline
    \rule{0pt}{1.15cm}输出母线电压 & 约 400 V 直流输出 & 满足后级 220 V/50 Hz 单相逆变所需母线电压，并预留调制和动态裕量 \\
    \hline
    \rule{0pt}{1.15cm}额定功率 & 约 200 W 等级 & 依据功率器件耐压、电感电流能力和散热条件综合确定 \\
    \hline
    \rule{0pt}{1.15cm}控制平台 & GD32G553、GD32C113、GD32VW553 & 主功率控制、BMS、无线监控分工 \\
    \hline
    \rule{0pt}{1.15cm}保护功能 & 过压、欠压、过流、过温、通信异常 & 硬件保护与软件状态机结合 \\
    \hline
  \end{tabular}
\end{table}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{images/主控板3D图.png}
  \caption{主功率控制板3D图}
  \label{fig:system_architecture}
\end{figure}
...
在 APP 层，系统通过统一调度器组织各模块运行，包括 BMS 数据转发、心跳帧输出和 HRTIMER PWM 任务等；在 Middleware 层，采用发布/订阅式数据总线组织系统内部数据流，使数据生产者与消费者解耦；在 Protocol 层，完成 CAN FD 帧解析与 UART AA55 帧封装；

在 HAL 层，则对 CAN、UART、HRTIMER 等外设进行统一抽象，使上层逻辑不直接依赖具体硬件实现。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.45\textwidth]{images/bms板3D图.png}
  \caption{bms控制板3D图}
  \label{fig:system_architecture}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{images/bms控制板原理图1.png}
  \caption{bms控制原理图1}
  \label{fig:system_architecture}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{images/bms控制板原理图2.png}
  \caption{bms控制原理图2}
  \label{fig:system_architecture}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\textwidth]{images/bms控制板原理图3.png}
  \caption{bms控制原理图3}
  \label{fig:system_architecture}
\end{figure}
...
BMS 的产品状态机主要包含 INIT FAIL、RUN 和 DEGRADED 三种状态。系统上电后，首先执行板级初始化和 AFE 最小唤醒，然后关闭 FET 通路并清除均衡，再进入完整配置流程，包括 16S 配置、CCGain 设置以及保护项装载。只有当首次测量有效、无严重故障且电芯状态正常时，系统才进入 RUN 状态。若运行过程中出现读取失败、保护触发、FET 应用失败或严重安全异常，则立即进入 DEGRADED 状态，执行均衡抑制和 FET 全关断，待故障恢复后再返回正常运行。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\textwidth]{images/三端口板原理图1.png}
  \caption{三端口板电路原理图}
  \label{fig:system_architecture}
\end{figure}
...
在本阶段，三端口功率变换部分已经完成电路仿真和功率板原理图及 PCB 设计。仿真主要围绕充电、放电和端口切换等典型工况展开，重点观察各端口电压电流变化、功率器件开关状态、耦合电感电流变化以及控制量调节过程，共仿真五个工作模式。
\subsubsection{光伏端向电池端充电工况仿真}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/充电仿真测试.png}
  \caption{模式一 充电测试仿真}
  \label{fig:system_architecture}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/恒流充电.png}
  \caption{模式一 闭环控制实现恒流充电}
  \label{fig:system_architecture}
\end{figure}

模式一用于验证三端口变换器在光伏端向电池端充电时的工作过程。该工况下，光伏输入 $V_{pv}$ 作为能量来源，电池端 $V_{bat}$ 作为充电对象，系统通过控制相关功率开关管的 PWM 占空比，使能量经电感和耦合磁性元件传递至电池端。

控制部分采用电池充电电流闭环控制。实际电池电流 $I_b$ 经移动平均滤波后与给定电流比较，误差送入 PI 控制器，控制器输出 PWM 占空比，从而调节光伏侧向电池侧传递的功率。该方式能够限制充电电流，减小充电过程中的电流冲击。

仿真中通过电压、电流检测点观察 $V_{pv}$、$V_{bat}$、$I_b$、电感电流和输出电压等波形，用于判断充电电流是否稳定、端口电压是否正常以及系统是否存在明显振荡。该仿真为后续功率板实物调试和充电控制参数整定提供参考。

\subsubsection{光伏端充电并稳压输出工况仿真}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{images/模式2.png}
  \caption{模式二 光伏端同时充电与输出仿真}
  \label{fig:system_architecture}
\end{figure}

模式二用于验证三端口变换器在光伏端同时向电池端充电和向输出端供电时的工作过程。该工况下，光伏输入 $V_{pv}$ 作为能量来源，一部分能量传递至电池端 $V_{bat}$，实现恒流充电；另一部分能量经耦合磁性元件和整流滤波支路传递至输出端，使输出电压 $V_o$ 稳定在约 $400\,\mathrm{V}$。

控制部分采用闭环调节方式。输出电压 $V_o$ 与给定值比较后送入 PI 控制器，控制器输出 PWM 占空比以调节主开关管导通时间，从而维持输出电压稳定。同时，电池电流 $I_b$ 用于观察充电状态，保证电池端处于恒流充电过程。

仿真中主要观察光伏侧电流、电池充电电流、输出电压、电感电流和开关驱动信号。若输出电压能够稳定在目标值附近，电池电流保持在设定范围内，则说明该模式下系统能够实现光伏供电、输出稳压和电池充电的协同运行。
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/模式2恒压恒流.png}
  \caption{模式二 对输出端恒压输出（上）对电池恒流充电（下）}
  \label{fig:system_architecture}
\end{figure}

\subsubsection{光伏端单独供电稳压输出工况仿真}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/模式3.png}
  \caption{模式三 光伏端单独输出输出仿真}
  \label{fig:system_architecture}
\end{figure}


模式三用于验证三端口变换器在光伏端单独向输出端供电时的工作过程。该工况下，光伏输入 $V_{pv}$ 作为唯一能量来源，电池端不参与主要能量传递，光伏侧能量经开关管、电感 $L_1$、耦合磁性元件 $Tr3$ 以及整流滤波支路传递至输出端，使输出电压 $V_o$ 稳定在约 $400\,\mathrm{V}$。

控制部分采用输出电压闭环控制。输出电压 $V_o$ 与给定值比较后形成误差，误差信号经 PI 控制器调节后生成 PWM 占空比，用于控制主功率开关管的导通时间，从而改变光伏侧向输出端传递的平均功率。当负载或输入条件发生变化时，控制器通过调整占空比维持输出电压稳定。

仿真中主要观察光伏侧电压电流、输出电压 $V_o$、电感电流和开关驱动信号等波形。若输出电压能够稳定在目标值附近，电感电流和开关波形无明显异常振荡，则说明该模式下系统能够实现光伏端单独供电和高压直流稳压输出。

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/模式3恒压.png}
  \caption{模式三 对输出恒压输出}
  \label{fig:system_architecture}
\end{figure}

\subsubsection{光伏功率不足时电池辅助供电工况仿真}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.6\textwidth]{images/模式4.png}
  \caption{模式四 光伏端与电池端共同输出}
  \label{fig:system_architecture}
\end{figure}


模式四用于验证光伏输入功率不足时，电池端自动放电并与光伏端共同向输出端供电的工作过程。该工况下，光伏输入 $V_{pv}$ 仍作为优先能量来源，当光伏侧功率不能满足输出端负载需求时，电池端 $V_{bat}$ 通过放电支路补充能量，使输出端电压 $V_o$ 维持在约 $400\,\mathrm{V}$。

控制部分以输出电压稳定为主要目标。输出电压 $V_o$ 与给定值比较后进入 PI 控制器，控制器调节主功率开关的 PWM 占空比，从而维持输出端电压稳定。同时，系统对光伏侧功率进行判断，当检测到光伏功率低于负载需求时，切换电池放电支路，使电池侧参与供能。通过这种方式，系统能够在光伏功率波动时保持输出端供电连续性。

仿真中主要观察光伏侧功率、电池放电电流、输出电压 $V_o$、电感电流以及开关驱动信号。若在光伏功率不足时电池电流能够自动进入放电状态，且输出电压仍稳定在目标值附近，则说明该模式下系统能够实现光伏与电池联合供电，并提高输出端供电稳定性。
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/模式4恒压.png}
  \caption{模式四 对输出恒压输出}
  \label{fig:system_architecture}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/模式4功率.png}
  \caption{模式四 电池端功率（上）与光伏端功率（下）}
  \label{fig:system_architecture}
\end{figure}

\subsubsection{电池端单独供电稳压输出工况仿真}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/模式5.png}
  \caption{模式五 电池端单独输出}
  \label{fig:system_architecture}
\end{figure}

图中电路用于验证三端口变换器在电池端单独向输出端供电时的工作过程。该工况下，光伏端不作为主要能量来源，储能电池 $V_{bat}$ 通过放电支路向输出端提供能量，经过开关管、电感和耦合磁性元件进行升压变换后，在输出端形成约 $400\,\mathrm{V}$ 的直流母线电压。

控制部分采用输出电压闭环控制。输出电压 $V_o$ 与给定值比较后送入 PI 控制器，控制器输出 PWM 占空比，用于调节电池侧放电通道的开关状态和能量传递量。当输出电压低于设定值时，控制器增大能量传递；当输出电压高于设定值时，控制器减小占空比，从而使输出端电压稳定在目标值附近。

仿真中主要观察电池放电电流、输出电压 $V_o$、电感电流以及开关驱动信号等波形。若电池端能够持续向输出端供能，且输出电压稳定在 $400\,\mathrm{V}$ 附近，则说明该模式下系统具备电池独立供电和离网稳压输出能力。
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/模式5恒压.png}
  \caption{模式五 对输出恒压输出}
  \label{fig:system_architecture}
\end{figure}
...
测试将按照由低压到高压、由开环到闭环、由单端口到多端口协同的顺序推进。首先验证隔离驱动、电源、采样和保护接口是否正常；随后在限压限流条件下测试单端口能量传递和开关波形；最后逐步开展充放电闭环、输出稳压和多模式切换测试。通过这种分阶段测试方式，可以降低功率级调试风险，并逐步验证三端口方案在实际硬件中的可行性。
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.65\textwidth]{images/三端口板3D图.png}
  \caption{三端口板3D图}
  \label{fig:system_architecture}
\end{figure}
