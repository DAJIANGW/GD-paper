...
系统测试围绕功能正确性、控制稳定性、电池保护有效性和无线监控可靠性展开。测试过程应覆盖单模块调试、功率级联调、整机运行和异常工况验证四个阶段。由于光储一体化系统涉及高压母线和交流输出，测试时需先在限压、限流和假负载条件下验证控制逻辑，再逐步提升至额定工作点。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{|p{0.22\linewidth}|p{0.34\linewidth}|p{0.32\linewidth}|}
    ...
    \hline
    交流输出 & 接入阻性或典型交流负载 & 输出电压、频率、波形畸变、带载能力 \\
    \hline
...
从设计目标看，系统性能评价应重点关注三类指标。第一类是能量转换指标，包括光伏端 MPPT 效果、整机效率、母线纹波和交流输出质量；第二类是动态控制指标，包括负载阶跃时的电压跌落、恢复时间和模式切换冲击；第三类是安全与可靠性指标，包括 BMS 保护动作、故障锁存、通信异常处理和温升裕量。
